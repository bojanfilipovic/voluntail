...
export function EditAnimalDialog({
  ...
  useEffect(() => {
    ...
    /* eslint-disable react-hooks/set-state-in-effect -- hydrate when dialog opens */
    setShelterId(animal.shelterId)
    setName(animal.name)
    setDescription(animal.description)
    setSpecies(animal.species)
    setStatus(animal.status)
    setPublished(animal.published)
    setImageUrl(animal.imageUrl ?? '')
    setExternalUrl(animal.externalUrl ?? '')
    setFormError(null)
